...
const setTimeState = () => {
	...
	if (el) {
		const timeNow = new Date(),
			...
			lang = GLOBALCONFIG.lang.sayhello;
		let text = '';
		if (hours >= 0 && hours <= 5) {
			text = lang.goodnight;
				...
				utils.scrollToDest(
					utils.getEleTop(
						document.getElementById(
							decodeURI((event.target.className === 'toc-text' ? event.target.parentNode.hash : event.target.hash).replace('#', ''))
						)
					),
					300
				);
	...
	static initTheme() {
		...
		const nowMode =
			cachedMode && (cachedMode === 'dark' || cachedMode === 'light')
				? cachedMode === 'dark' && isLightMode
					? 'dark'
					: cachedMode === 'light' && isDarkMode
					? 'light'
					: cachedMode
				: isDarkMode
				? 'dark'
...
class hightlight {
	static createEle(langEl, item) {
		...
		const itemHeight = item.clientHeight,
			$table = item.querySelector('table');
